...
print_header "Запуск YARN на нейм-ноде"
ssh "$SSH_USER@team-4-nn" "cd /home/$SSH_USER/hadoop-3.4.0 && sbin/start-yarn.sh" || error_exit "Не удалось запустить YARN."
check_success

# Настройка Nginx для проксирования YARN
read -p "Настроить Nginx для проксирования YARN? (y/n): " setup_nginx
if [[ "$setup_nginx" == "y" ]]; then
    NGINX_DIR="/etc/nginx/sites-available"
    PORT="8088"

    print_header "Настройка Nginx для YARN на порту $PORT"
    ...
    }' | sudo tee \"$NGINX_DIR/yarn\" > /dev/null" || error_exit "Не удалось настроить конфигурацию nginx для YARN."

    ssh -t team@team-4-jn "sudo ln -sf \"$NGINX_DIR/yarn\" \"/etc/nginx/sites-enabled/yarn\" && sudo systemctl reload nginx" || error_exit "Не удалось активировать конфигурацию и перезагрузить nginx."
    check_success
fi