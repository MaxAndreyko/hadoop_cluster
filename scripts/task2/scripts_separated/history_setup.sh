...
print_header "Запуск History Server на нейм-ноде"
ssh "$SSH_USER@team-4-nn" "cd /home/$SSH_USER/hadoop-3.4.0 && bin/mapred --daemon start historyserver" || error_exit "Не удалось запустить History Server."
check_success

# Настройка Nginx для проксирования History Server
read -p "Настроить Nginx для проксирования History Server? (y/n): " setup_nginx
if [[ "$setup_nginx" == "y" ]]; then
    NGINX_DIR="/etc/nginx/sites-available"
    PORT="19888"

    print_header "Настройка Nginx для History Server на порту $PORT"
    ...
    }' | sudo tee \"$NGINX_DIR/historyserver\" > /dev/null" || error_exit "Не удалось настроить конфигурацию nginx для History Server."

    ssh -t team@team-4-jn "sudo ln -sf \"$NGINX_DIR/historyserver\" \"/etc/nginx/sites-enabled/historyserver\" && sudo systemctl reload nginx" || error_exit "Не удалось активировать конфигурацию и перезагрузить nginx."
    check_success
fi