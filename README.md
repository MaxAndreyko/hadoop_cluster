...
Переходим в папку со скриптами `scripts/task1` к ПЗ №1 и делаем их исполняемыми:
```bash
cd scripts/task1 && chmod +x ./*.sh
```
### II. Создание подключения по SSH
1. Поместить файл с именем hosts.txt в папку со скриптом `create_users.sh`. В файле необходимо указать внутренние IP хостов (нод) для team-4:
```
192.168.1.18 team-4-jn
192.168.1.19 team-4-nn
192.168.1.20 team-4-dn-0
192.168.1.21 team-4-dn-1
```
где:
- team-4-jn - джамп-нода
- team-4-nn - нейм-нода
- team-4-dn-0 - дата-нода 0
- team-4-dn-1 - дата-нода 1
2. Для создания подключения по ssh запускаем скрипт `create_users.sh`:
```bash
sudo ./create_users.sh
```
3. Далее необходимо ввести имя пользователя (например, `hadoop`) и пароль к нему. Послее ввода ожидаем завершение выполенния скрипта;
4. При успешном выполнении всех команд скрипта в терминале должно отобразиться `"Завершение автоматического создания подключения по SSH."`.
...
4. При успешном выполнении всех команд скрипта в терминале должно отобразиться `"Завершение автоматического разворачивания hadoop."`.
   
## ПЗ №2
### I. Подготовка к запуску скриптов
Переходим в папку со скриптами `scripts/task2` к ПЗ №2 и делаем их исполняемыми:
```bash
cd scripts/task2 && chmod +x ./*.sh
```
